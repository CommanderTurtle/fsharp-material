  ...
  build-and-deploy:
    ...
    steps:
      ...
      - name: Generate configs
        ...
        run: |
          if [ -f "pyproject.fs" ]; then dotnet fsi -e "#load \"pyproject.fs\"; open Docs.PyProject; System.IO.File.WriteAllText(\"pyproject.toml\", render())"; fi
          if [ -f "mkdocs.fs" ]; then dotnet fsi -e "#load \"mkdocs.fs\"; open Docs.MkDocs; System.IO.File.WriteAllText(\"mkdocs.yml\", render())"; fi
      ...
      - name: Deploy to docs repo
        ...
        run: |
          ...
          echo "docs.shel.sh" > CNAME
          git add . && git commit -m "Deploy docs [skip ci]" || true
